...
import { MapPin, Navigation, RefreshCw, AlertCircle, User } from "lucide-react"
import "./Tracking.css"
...
export const Tracking: React.FC = () => {
    ...
    const courierMarkers: CourierMarker[] = couriers.map((courier) => ({
        ...
        profilePhoto: courier.profilePhoto,
    }))
    ...
    return (
        <div className="tracking-page">
            <div className="tracking-header">
                ...
                <button className="refresh-btn" onClick={fetchCouriers} disabled={isLoading} title="Actualizar ahora">
                ...
                <div className="tracking-sidebar">
                    <div className="sidebar-section">
                        <div className="sidebar-header">
                            ...
                            {lastRefresh && <span className="last-refresh">Actualizado: {lastRefresh.toLocaleTimeString()}</span>}
                        </div>
                                    ...
                                    >
                                        <div className="courier-card-header">
                                            <div className="courier-avatar">
                                                ...
                                                <div className={`courier-avatar-placeholder ${courier.profilePhoto ? 'hidden' : ''}`}>
                                                    <User size={20} />
                                                </div>
                                            </div>
                                            <div className="courier-info">
                                                <h4 className="courier-name">{courier.name}</h4>
                                                {courier.currentTaskName && <span className="courier-task">{courier.currentTaskName}</span>}
                                            </div>
                                        </div>
                                        <div className="courier-card-footer">
                                            ...
                                            <span className="courier-time">{courier.lastUpdate.toLocaleTimeString()}</span>
                                        </div>
                                    </div>
                                ))
                            )}
